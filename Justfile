...
# Build Image
[group('Image')]
build $image="kinoite" $tag="latest" $flavor="main" rechunk="0" ghcr="0" pipeline="0" $kernel_pin="":
    #!/usr/bin/bash
    ...
    LABELS+=("--label" "io.artifacthub.package.readme-url=https://raw.githubusercontent.com/ublue-os/kinoite/refs/heads/main/README.md")
    ...
    LABELS+=("--label" "org.opencontainers.image.source=https://raw.githubusercontent.com/ublue-os/kinoite/refs/heads/main/Containerfile")
    LABELS+=("--label" "org.opencontainers.image.url=https://getaurora.dev")
    LABELS+=("--label" "org.opencontainers.image.vendor={{ repo_organization }}")
    LABELS+=("--label" "io.artifacthub.package.deprecated=false")
    LABELS+=("--label" "io.artifacthub.package.keywords=bootc,fedora,kinoite,ublue,universal-blue")
...
[group('Image')]
build-rechunk image="kinoite" tag="latest" flavor="main" kernel_pin="":
    @just build {{ image }} {{ tag }} {{ flavor }} 1 0 0 {{ kernel_pin }}

# Build Image with GHCR Flag
[group('Image')]
build-ghcr image="kinoite" tag="latest" flavor="main" kernel_pin="":
    #!/usr/bin/bash
    if [[ "${UID}" -gt "0" ]]; then
...
[group('Image')]
build-pipeline image="kinoite" tag="latest" flavor="main" kernel_pin="":
    #!/usr/bin/bash
...
[group('Image')]
[private]
rechunk $image="kinoite" $tag="latest" $flavor="main" ghcr="0" pipeline="0":
    #!/usr/bin/bash
    ...
    LABELS="
        ...
        io.artifacthub.package.keywords=bootc,fedora,kinoite,ublue,universal-blue
        io.artifacthub.package.logo-url=https://avatars.githubusercontent.com/u/120078124?s=200&v=4
        io.artifacthub.package.maintainers=[{\"name\": \"castrojo\", \"email\": \"jorge.castro@gmail.com\"}]
        io.artifacthub.package.readme-url=https://raw.githubusercontent.com/ublue-os/kinoite/refs/heads/main/README.md
        org.opencontainers.image.created=$(date -u +%Y\-%m\-%d\T%H\:%M\:%S\Z)
        org.opencontainers.image.license=Apache-2.0
        org.opencontainers.image.source=https://raw.githubusercontent.com/ublue-os/kinoite/refs/heads/main/Containerfile
...
[group('Image')]
load-rechunk image="kinoite" tag="latest" flavor="main":
    #!/usr/bin/bash
    set -eou pipefail
...
# Run Container
[group('Image')]
run $image="kinoite" $tag="latest" $flavor="main":
    #!/usr/bin/bash
    set -eoux pipefail
...
# Build ISO
[group('ISO')]
build-iso $image="kinoite" $tag="latest" $flavor="main" ghcr="0" pipeline="0":
    #!/usr/bin/bash
    set -eoux pipefail
    ...
    FLATPAK_DIR_SHORTNAME="kinoite_flatpaks"
...
[group('ISO')]
build-iso-ghcr image="kinoite" tag="latest" flavor="main":
    @just build-iso {{ image }} {{ tag }} {{ flavor }} 1

# Run ISO
[group('ISO')]
run-iso $image="kinoite" $tag="latest" $flavor="main":
    #!/usr/bin/bash
    set -eoux pipefail
...
# Secureboot Check
[group('Utility')]
secureboot $image="kinoite" $tag="latest" $flavor="main":
    #!/usr/bin/bash
    set -eou pipefail
...
[group('Utility')]
[private]
fedora_version image="kinoite" tag="latest" flavor="main" $kernel_pin="":
    #!/usr/bin/bash
    set -eou pipefail
    just validate {{ image }} {{ tag }} {{ flavor }}
...
[group('Utility')]
[private]
image_name image="kinoite" tag="latest" flavor="main":
    #!/usr/bin/bash
    set -eou pipefail
    just validate {{ image }} {{ tag }} {{ flavor }}
    if [[ "{{ flavor }}" =~ main ]]; then
...
# Generate Tags
[group('Utility')]
generate-build-tags image="kinoite" tag="latest" flavor="main" kernel_pin="" ghcr="0" $version="" github_event="" github_number="":
    #!/usr/bin/bash
    set -eou pipefail
...
retag-nvidia-on-ghcr working_tag="" stream="" dry_run="1":
    ...
    for image in kinoite-nvidia-open kinoite-nvidia kinoite-dx-nvidia kinoite-dx-nvidia-open; do
